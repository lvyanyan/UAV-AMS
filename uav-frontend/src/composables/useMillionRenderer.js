/**
 * 百万级点渲染器 —— 懒加载分帧增长（移植自 million-demo v4，已验证 100 万架）
 *
 * 不在启动时一次性预分配 N 个 primitive（那会阻塞主线程数百毫秒 → 假死），
 * init 瞬间完成，render 时按需"每帧长一点"（GROW_PT/帧），把分配摊到多帧里。
 *
 * 两个集合（帧类型即 LOD，互斥）：
 *   _ptCol  PointPrimitiveCollection  MAX_PT=20万  cell聚合帧 / 抽稀后raw  按密度或告警上色
 *   _bbCol  BillboardCollection       MAX_BB=1万   低空raw帧               图标+航向旋转
 */
...
export function useMillionRenderer(viewerRef) {
  let _ptCol = null
  let _bbCol = null
  let _icon = null
  ...
  function init() {
    ...
    _ready = true
    console.log('[Million] 渲染器就绪（懒加载模式，按需分帧增长）')
  }
