/**
 * 百万级机群二进制帧解析 Worker（移植自 million-demo v5，协议一致）
 *
 * 服务端（uav-realtime :8091/fleet）按相机视野推两种帧：
 *   raw  (magic 0x55534156 "UAVV")：低空视野内原始点，16头 + N×20B [lon,lat,alt,hdg,alert]
 *   cell (magic 0x55534143 "USAC")：高空网格聚合，16头 + M×16B [lon,lat,count,alertAvg]
 *
 * Worker 按 magic 分流，统一输出 pos+meta(+ll)：
 *   raw  → ECEF + 告警/航向/经纬度
 *   cell → ECEF + [count当作密度信号, 0 heading]（点大小∝count 由渲染器处理）
 *
